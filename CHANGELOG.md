...
## 2026-09-22 — แยกรอบคำถอดเสียงและรับเสียงต่อระหว่างรอเครื่องมือ

- `app/providers/gemini.py`, `app/session.py`, `client/voice-preview.html`: ส่งเหตุการณ์เริ่มพูดจาก local VAD และรหัสรอบคำถอดเสียง แยกบรรทัดผู้ใช้แต่ละรอบพร้อมรองรับข้อความมาช้า; แยกลูปเครื่องมือออกจากการรับเสียง คงลำดับเครื่องมือ และเก็บทุกส่วนของข้อความที่มี tool call พร้อมเสียง/ข้อความ
- ดูแลงานเบื้องหลังเมื่อปิดสาย ป้องกันผลเครื่องมือจาก connection เก่าเข้า connection ใหม่ และข้ามงานในคิวที่โมเดลยกเลิก; คงค่าไมค์, VAD, HALF_DUPLEX และโมเดลตามที่ผู้ใช้จูนไว้
- `app/providers/base.py`, `tests/test_gemini_streaming.py`, `tests/test_voice.py`, `tests/client_preview_transcript.cjs`, `tests/test_voice_preview_page.py`: บันทึก optional utterance_id และเพิ่มการทดสอบพฤติกรรมจริงของ event handler/provider/WebSocket ด้วยเสียงและเครื่องมือจำลอง ไม่เรียกฮาร์ดแวร์
- เหตุผล: Gemini ไม่เคยส่ง speech_started ให้หน้า preview จึงรวมข้อความหลายรอบ และการรอผลเครื่องมือในลูปรับเสียงทำให้เสียง/เหตุการณ์พูดแทรกรอไปด้วย
- ผลตรวจจริง: ชุดทดสอบทั้งโปรเจกต์ผ่าน 1,651 รายการ มี warning เดิมจาก Starlette/httpx 1 รายการ; ทดสอบ provider/WebSocket และรัน JavaScript handler จริงยืนยันการแยกรอบ ข้อความมาช้า เสียงผ่านระหว่างเครื่องมือรอผล ลำดับเครื่องมือ การยกเลิก และการปิดสาย รีสตาร์ตเซิร์ฟเวอร์พอร์ต 8001 แล้ว health ตอบ 200 และ WebSocket ตอบ ready พร้อม half_duplex=true; สายทดสอบถูกแทนที่โดยอีกแท็บก่อนตรวจคำทักทายครบ แต่ log ของสายที่เชื่อมภายหลังยืนยันว่ามีเสียงตอบกลับและจบรอบได้ ยังไม่ได้ตรวจด้วยไมค์/ลำโพงหุ่นจริง
...
- `tests/test_voice_preview_page.py`: เพิ่ม regression ยืนยันว่า hit area ใช้ transform เดียวกับตัวละคร ชิ้นภาพภายในไม่แปลตำแหน่งซ้ำ และขอบจอยังเหลือพื้นที่จับเพื่อลากต่อ
- เหตุผล: ก่อนแก้มีเพียงภาพ Rive/rig ที่ถูกเลื่อน แต่ `.mascot-wrap` ซึ่งรับ `pointerdown` ยังอยู่กลางจอ เมื่อปล่อย Emma ที่ด้านข้างจึงเห็นตัวละครแต่คลิกตัวละครเพื่อเริ่มลากรอบใหม่ไม่ได้
- ผลตรวจจริง: เปิดหน้าพรีวิวแล้วลาก Emma จากกลางไปชิดซ้าย ปล่อย และจับจากตำแหน่งใหม่ลากกลับได้สำเร็จ จากนั้น reload คืนตำแหน่งกลาง; ชุดทดสอบ preview, Android packaging และ JavaScript syntax ผ่าน 33 รายการ มีเพียง warning เดิมจาก Starlette/httpx และ pytest cache
...
- `app/tools/project_knowledge.py`, `app/tools/slide_search.py`, `app/tools/units.py`, `app/prompts.py`: ผูก vocabulary cache กับ project/path/content และไม่ส่ง routing hint ที่ไม่ได้อนุมัติให้ customer-text ranker; local inventory cache ตรวจ SHA-256 ของเนื้อหาจริงแม้ mtime/ขนาดไม่เปลี่ยน, live cache ผูก project, ผังอ่าน manifest ใหม่แทน cache ข้าม project และเอา legacy `units.find` ที่คืน raw row โดยไม่มี gate ออกเพราะไม่มีผู้เรียก; explicit facts path ต้องมี `source_id=project_facts` กับ project/approval ปัจจุบัน
- `app/tools/registry.py`, `app/tools/websearch.py`, `app/tools/documents.py`, `app/session.py`: ทุก tool result ตรวจ foreign project; exception/TypeError คืน error ทั่วไปไม่ส่งข้อความภายใน; provider setup rejection ยังเตือน `REJECTED / guardrails unavailable` แต่ไม่ echo raw error body ไป UI; condo profile ไม่ให้เปิด web/personal/computer/mydocs ผ่าน `TOOL_GROUPS` จนมี policy เฉพาะ และ direct websearch fail closed; printer stderr/path ไม่กลับไป provider หรือ turn log
- `tests/test_runtime_egress.py`, `tests/test_review_fixes.py`, `tests/test_tools.py`, `tests/test_knowledge_policy.py`, `tests/test_voice.py`, `tests/test_websearch_computer.py`, `tests/test_documents.py`, `docs/runtime-egress-map.md`: เพิ่ม regression ที่ provider serialized payload ทั้ง Gemini/OpenAI, UI event, Canva command, guessed URL, cached project switch, explicit source ผิด, optional config และ printer diagnostic; ปรับ fixtures ของ approved facts ให้ใช้ source ID จริง และทดสอบ web/computer ใน Emma profile; ทำ egress map กับ production validation matrix แยก live smoke/M1. ไม่แก้ approval ของข้อมูลจริง
- ผลตรวจจริง: ชุด focused ก่อนเก็บ printer/HTTP **224 ผ่าน / 1 warning**, ชุดกว้าง **440 ผ่าน / 1 warning**, ชุด cache/static inventory **61 ผ่าน / 1 warning**, audit CI **0 errors / 3 warnings** ที่เป็นข้อมูลยังไม่อนุมัติ, `compileall` และ `git diff --check` ผ่าน; suite เต็มล่าสุด **1,599 ผ่าน / 1 known failure / 1 warning** (`tests/test_voice.py::test_system_instruction_stays_short` 6,460 เทียบเกณฑ์ 4,700 เดิม), ไม่มี skip หรือ failure ใหม่. ยังไม่ได้ทดสอบ Canva remote, เครื่องพิมพ์จริง, live provider หรือ render ACK
