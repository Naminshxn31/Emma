...
> บันทึกประวัติ ณ 8 กันยายน 2026: ตัวจำลองก่อนหุ่นมาที่กล่าวถึงด้านล่างถูกเลิกใช้และลบออกแล้วเมื่อ 17 กันยายน 2026 รายการนี้ไม่ใช่คู่มือเปิดระบบปัจจุบัน
...
ตอนนี้ `app/tools/robot_link.py` และ handler หุ่นจริงใน `app/session.py` ยังไม่ได้ใช้ contract command ID/ACK ของตัวจำลองครบ จึงควรทำ conformance suite ชุดเดียวให้ simulator และ adapter ผ่านก่อนเริ่มเดินจริง รายละเอียด vendor API ให้ยึด [คู่มือ integration ในโปรเจกต์](../robot-integration.md) และตรวจ AAR จริง ไม่อ้างว่า ROS protocol คือ Aobo protocol
